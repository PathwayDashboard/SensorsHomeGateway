...
//					s.setStepsNumber(run.);//problemi con gli steps - calcolare in base a passo medio 0,75m a passo
					...
					int steps=0;
					if (dsummary!=null && dsummary.getTotalDistance() != null){
...
//			return profile.toString();
			...
			CloudLogger.logger.fine(e.getResponseBody());
				UnauthorizedException ex=new UnauthorizedException(e.getCode(), e.getResponseBody());
				CloudLogger.logger.fine(ex.toString());
				throw ex;
		}
		}
		else{
...
//		}
		...
		try {
			...
			if (startDate != null && startDate.before(startDate)){
				resp=summaryApi.summariesGet(SUMMARY_PERIOD, startDate, endDate, null, null);
			}
